...
test('parseArgs accepts profile and source overrides', () => {
  ...
  assert.deepEqual(parseArgs(['--profile', 'lab', '--source', 'link:../dsh-file-picker']), {
    command: 'install',
    profile: 'lab',
    source: 'link:../dsh-file-picker',
  })
...
test('applyManifest is idempotent and keeps existing bundles', () => {
  const first = applyManifest({
    dependencies: { 'dsh-attention': 'link:/tmp/attention' },
    dsh: { profile: { bundles: ['@deepseek-ai/dsh-web-app', 'dsh-attention'] } },
  }, 'link:/tmp/picker')
  const second = applyManifest(first, DEFAULT_SOURCE)
  assert.equal(first.dependencies[PACKAGE_NAME], 'link:/tmp/picker')
  ...
  assert.equal(second.dependencies[PACKAGE_NAME], DEFAULT_SOURCE)
})
...
test('removeManifest strips the dependency and bundle entry, and is idempotent', () => {
  ...
  assert.ok(!('dsh-file-picker' in removed.dependencies))
  assert.equal(removed.dependencies['dsh-attention'], 'link:/tmp/attention')
  assert.deepEqual(removed.dsh.profile.bundles, ['@deepseek-ai/dsh-web-app', 'dsh-attention'])
  assert.deepEqual(removeManifest(removed), removed)
...
test('describeStatus reports dependency and bundle membership', () => {
  ...
  assert.deepEqual(describeStatus(applyManifest({}, DEFAULT_SOURCE)), {
    installed: true,
...
async function makeProfile() {
  const home = await mkdtemp(join(tmpdir(), 'dsh-file-picker-test-'))
...
test('install, repeat install, status, and uninstall work end to end', async () => {
  ...
  try {
    ...
    await run(['--source', 'link:/tmp/picker'], { home, installDeps })
    manifest = JSON.parse(await readFile(packagePath, 'utf8'))
    assert.equal(manifest.dependencies[PACKAGE_NAME], 'link:/tmp/picker')
...
test('install restores the manifest when dependency installation fails', async () => {
  ...
  try {
    ...
    await assert.rejects(
      () => run([], { home, installDeps: async () => { throw new Error('pnpm boom') } }),
      /pnpm boom/,
    )
    assert.equal(await readFile(packagePath, 'utf8'), before)
  } finally {
...
test('uninstall restores the manifest when dependency installation fails', async () => {
  ...
  try {
    ...
    await assert.rejects(
      () => run(['uninstall'], { home, installDeps: async () => { throw new Error('pnpm boom') } }),
      /pnpm boom/,
    )
