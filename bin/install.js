...
export const PACKAGE_NAME = 'dsh-file-picker'
export const DEFAULT_SOURCE = 'github:shaomingbo/dsh-file-picker#v0.2.1'
...
export function parseArgs(argv) {
  const result = {
    command: 'install',
    profile: 'web',
    source: process.env.DSH_FILE_PICKER_SOURCE || DEFAULT_SOURCE,
  }
...
Commands:
  install     Add the file-picker bundle to the profile (default)
  status      Show whether the file-picker bundle is installed
  uninstall   Remove the file-picker bundle from the profile (idempotent)

Options:
  --profile <name>   Target DSH profile (default: web)
  --source <source>  Package source (default: ${DEFAULT_SOURCE},
                     override with the DSH_FILE_PICKER_SOURCE environment variable)
...
export async function run(argv = process.argv.slice(2), deps = {}) {
  ...
  const dshHome = resolve(deps.home ?? process.env.DSH_HOME ?? join(homedir(), '.dsh'))
  ...
  if (options.command === 'uninstall') {
    ...
    console.log('Restart DSH and refresh the existing Web GUI to unload the bundle.')
    return
  }
  ...
  console.log('Restart DSH and refresh the existing Web GUI to load the bundle.')
}
